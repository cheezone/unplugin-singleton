...
function baseUrlFromAddr(
  ...
  return `${protocol}://${host}:${addr.port}`.replace(/\/+$/, '')
}
...
function readDevLockFile(root: string): { baseUrl: string; port: number } | null {
  ...
  if (!payload || payload.port <= 0 || payload.port > 65535) return null
  return { baseUrl: payload.baseUrl.replace(/\/+$/, ''), port: payload.port }
}
...
function fallbackBaseUrlAndPort(config: ResolvedConfigLike): { baseUrl: string; port: number } {
  ...
  const baseUrl = `${protocol}://${host}:${port}`.replace(/\/+$/, '')
  return { baseUrl, port }
}

function isPidAlive(pid: number | string | null | undefined): boolean {
  if (pid == null || (typeof pid !== 'number' && typeof pid !== 'string')) return false
  const n = Number(pid)
...
function setupLockOnServer(
  ...
  if (!httpServer) {
    ...
    const doWriteLock = (payload: LockPayload) => {
...
export const unpluginFactory = (_options?: unknown, _meta?: unknown) => createPluginBody()
export const unplugin = /* #__PURE__ */ createUnplugin(unpluginFactory)

export { readDevLockFile, readExistingLock, tryAcquireLockSync, isPidAlive, lockPaths, fallbackBaseUrlAndPort }
