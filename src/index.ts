  ...
  const host =
    h === true || h === '0.0.0.0' || h === '::'
      ...
      : typeof h === 'string'
        ...
        : 'localhost';
  const fromEnv =
    process.env.PORT != null && process.env.PORT !== '' ? Number(process.env.PORT) : Number.NaN;
  const fromConfig =
...
interface ViteServer {
  config: {
    ...
    server?: ResolvedConfigLike['server'];
  };
  httpServer?: import('node:http').Server | null;
}

function setupLockOnServer(server: ViteServer, lockPath: string, serverLabel: string): void {
...
export const unpluginFactory = (
  _options?: unknown,
  _meta?: unknown,
): ReturnType<typeof createPluginBody> => createPluginBody();
